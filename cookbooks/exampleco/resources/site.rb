...
#    performAction=%x{curl -s -k -u l2007:Sahasra14$ -X POST -H "Content-Type: application/json" -d '{"uuid": "e2686b82-f8d7-436e-b4cc-cfae132d5fa0", "actionCode": "uploadonly", "ignoreInitialState": "false"}' https://ocloud-mintpress.wpdev.mintpress.io/REST/ServiceCatalog/performAction}
    ...
    response=%x{curl -s -k -u l2007:Sahasra14$ https://ocloud-mintpress.wpdev.mintpress.io/REST/ServiceCatalog/get?uuid="#{new_resource.uuid}"}
    ...
    Chef::Log.info "UUID is #{uuid} triggered Runtime Plan #{buildResultKey} and the Job execution state is #{parsed["lifeCycleState"]} and Job status is #{parsed["buildState"]} time taken for execution #{parsed["buildDurationInSeconds"]/60} mins and #{parsed["buildDurationInSeconds"]%60} secs"
